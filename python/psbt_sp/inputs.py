...
def get_input_script_pubkey(input_fields: dict) -> Optional[bytes]:
    ...
    if PSBTKeyType.PSBT_IN_WITNESS_UTXO in input_fields:
        witness_utxo = input_fields[PSBTKeyType.PSBT_IN_WITNESS_UTXO]
    ...
    elif PSBTKeyType.PSBT_IN_NON_WITNESS_UTXO in input_fields:
        non_witness_utxo = input_fields[PSBTKeyType.PSBT_IN_NON_WITNESS_UTXO]
        # Get the output index from PSBT_IN_OUTPUT_INDEX field
        if PSBTKeyType.PSBT_IN_OUTPUT_INDEX in input_fields:
            output_index_bytes = input_fields[PSBTKeyType.PSBT_IN_OUTPUT_INDEX]
...
def validate_input_eligibility(
    ...
    if is_p2sh(script_pubkey):
        if PSBTKeyType.PSBT_IN_REDEEM_SCRIPT in input_fields:
            redeem_script = input_fields[PSBTKeyType.PSBT_IN_REDEEM_SCRIPT]
