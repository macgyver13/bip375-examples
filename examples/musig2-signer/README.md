...
A grant organization wants to pay grantees using Silent Payments to avoid the operational burden of managing unique addresses for each payment batch. The treasury holding the funds is secured by an aggregated MuSig2 wallet (e.g. 3-of-3), with keys held on hardware signing devices.
...
A taproot key-spend sighash is computed over the transaction **after** the Silent Payment output scripts have been materialized in `PSBT_OUT_SCRIPT` (step 3) — the sighash commits to those output scripts (BIP-341 `sha_outputs`), so it is only well-defined once they are filled in. Each party then uses its stored secret nonce, the aggregated pubnonce, and this sighash to emit a partial MuSig2 signature into `PSBT_IN_MUSIG2_PARTIAL_SIG`. Note that the partial signatures (and the final aggregate) commit to this standard BIP-341 sighash; Silent Payments only changes the message the *nonce* is bound to, not the message the *signature* covers.
