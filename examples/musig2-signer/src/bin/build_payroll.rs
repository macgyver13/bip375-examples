...
//! Their account fingerprints and m/48'/1'/0'/3' origins are wired into the PSBT so
...
/// Write a `PSBT_IN_/OUT_TAP_BIP32_DERIVATION` entry (native `tap_key_origins`) for
/// `xonly` on input 0 and the change output, replacing the dropped spdk helpers.
...
/// Account-level BIP-48 derivation path: m/48'/1'/0'/3'.
...
struct Cosigner {
    ...
    xfp: [u8; 4],
    /// Extended public key at m/48'/1'/0'/3' from the Coldcard simulator fixed seed.
    xpub_str: &'static str,
}
...
#[allow(dead_code)] // alternate recipient set, wired in by uncommenting in main()
...
fn build_descriptor() -> String {
    let parts: Vec<String> = COSIGNERS_TEST
        ...
        .map(|c| {
            ...
            format!("[{}/48h/1h/0h/3h]{}", xfp_hex, c.xpub_str)
        })
        .collect();
...
fn main() -> Result<()> {
    ...
    let change_idx = psbt
        .outputs
        .iter()
        .position(|o| o.sp_v0_info.is_none())
    ...
    // Print computed SP output scripts for visual verification. Outputs are
    ...
    for (i, output) in psbt.outputs.iter().enumerate() {
        ...
        if let Some((addr, amount)) = recipients
            .iter()
            .find(|(a, _)| a.get_scan_key() == scan && a.get_spend_key() == spend)
        {
            println!(
                "  [{i}] {} BTC  {}",
                amount.to_btc(),
