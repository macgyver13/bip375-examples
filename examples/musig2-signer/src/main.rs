...
fn run_cli() -> anyhow::Result<()> {
    ...
    // Outputs are shuffled by the constructor (BIP-375); find the SP output by
    // its sp_v0_info rather than assuming a position.
    let sp_output_idx = psbt
        .outputs
        .iter()
        .position(|o| o.sp_v0_info.is_some())
        .ok_or_else(|| anyhow::anyhow!("no silent payment output present"))?;
    let output_script = psbt.outputs[sp_output_idx].script_pubkey.clone();
    println!(
        "SP output script: {}",
    ...
    for (name, _, _) in &parties {
        for entry in &partial_shares {
            let proof = psbt::core::utils::to_rust_dleq(entry.dleq_proof);
            let ok = psbt::verify_dleq_proof(
                &secp,
            ...
            .map_err(|e| anyhow::anyhow!("DLEQ verify: {e:?}"))?;
            if !ok {
                bail!(
    ...
    println!("Aggregated Schnorr signature: {}", hex::encode(sig_bytes));
