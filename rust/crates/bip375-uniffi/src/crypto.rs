...
// pub fn bip352_compute_ecdh_share(privkey: Vec<u8>, pubkey: Vec<u8>) -> Result<Vec<u8>, Bip375Error> {
...
//     let share = crypto::bip352::compute_ecdh_share(&sk, &pk)?;
...
//     let proof = crypto::dleq::dleq_generate_proof(&sk, &pk, &aux_bytes)?;
//     Ok(proof)
// }
...
//     let result = crypto::dleq::dleq_verify_proof(&pk_a, &pk_b, &pk_c, &proof)?;
//     Ok(result)
// }
...
//         deserialize(&tx).map_err(|_| Bip375Error::InvalidData)?;
