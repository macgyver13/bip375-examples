...
use spdk_core::psbt::{
    ...
    roles::{
        ...
        extract_transaction, finalize_inputs, Bip32Derivation,
    },
};
...
/// Run finalize_inputs to derive the SP output script from the aggregated ECDH shares.
pub fn derive_sp_output(
    secp: &Secp256k1<secp256k1::All>,
    psbt: &mut SilentPaymentPsbt,
) -> Result<()> {
    finalize_inputs(secp, psbt)?;
    Ok(())
}
