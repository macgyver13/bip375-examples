...
/// Validate a DNSSEC proof and extract payment information
///
/// This is a convenience wrapper that validates the proof and extracts
/// Bitcoin payment instructions from the TXT records.
///
/// # Returns
/// Result containing (dns_name, bitcoin_uri) or error message
pub fn validate_and_extract_payment_info(proof_bytes: &[u8]) -> Result<(String, String), String> {
    let (dns_name, txt_records) = validate_dnssec_proof(proof_bytes)?;

    // Look for Bitcoin payment instruction in TXT records
    // BIP-353 format: bitcoin:<address>?... or bitcoin:?lno=<lightning_offer>
    for record in txt_records {
        if record.starts_with("bitcoin:") {
            return Ok((dns_name, record));
        }
    }

    Err(format!(
        "No Bitcoin payment instruction found in DNSSEC-validated TXT records for '{}'",
        dns_name
    ))
}
